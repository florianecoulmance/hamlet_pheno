...
legend_geo <- function(info_table, gen = FALSE) {
  ...
  # species name
  geom_text(aes(x = col, y = y_text, label = Locations), size = 4.5, hjust = 1, vjust = 2, fontface = "bold") +
  theme_void() +
...
pca_plot <- function(pca_data, pc_first, pc_second, species_info, geo_info, var, color_by = "species", extract_legend = FALSE, legend_rows = 2) {
  ...
  p_annot <- annotate_figure(
    p,
    top = text_grob(title_val, color = "black", face = "bold", size = 20, x = unit(0, "lines"))#, fig.lab.pos = "top.left"
  )
